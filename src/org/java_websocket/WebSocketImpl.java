...
public class WebSocketImpl implements WebSocket {
	...
	private void decodeFrames( ByteBuffer socketBuffer ) {
		...
		try {
			...
			for( Framedata f : frames ) {
				...
				} else if( curop == Opcode.BINARY || curop == Opcode.TEXT ) {
					try {
	...
	public void send( byte[] bytes, int offset, int len ) throws IllegalArgumentException , WebsocketNotConnectedException {
		send( ByteBuffer.wrap( bytes, offset, len ) );
	}
