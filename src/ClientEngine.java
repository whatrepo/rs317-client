...
//			BufferedImage image = ImageIO.read(new URL("http://www.sythescape.com/Media/icon.png"));
...
//				trayIcon.displayMessage(ClientConstants.CLIENT_NAME, "Sythescape client has been started!", MessageType.INFO);
//			}
//		}
	}
...
/*		if (gameFrame == null) {
	...
	public final void keyPressed(KeyEvent keyevent) {
		...
		if (i == KeyEvent.VK_T && Client.controlIsDown ) {

			if (isPasting) {
				// skip
			} else {

				final ClientEngine[] thiz = new ClientEngine[1];
				thiz[0] = this;
				new Thread(new Runnable() {
					@Override
					public void run() {
						thiz[0].isPasting = true;

						for (int n = 1; n <= 10000; n++) {
							String clip = "::conf 108 " + n;
							System.out.println(clip);
							if (clip != null) {
								for (int i = 0; i < clip.length(); i++) {
									thiz[0].keyPressed(new KeyEvent(thiz[0].gameFrame, 0, 0, 0, KeyEvent.getExtendedKeyCodeForChar(clip.charAt(i)), clip.charAt(i)));
									thiz[0].keyReleased(new KeyEvent(thiz[0].gameFrame, 0, 0, 0, KeyEvent.getExtendedKeyCodeForChar(clip.charAt(i)), clip.charAt(i)));
									try {
										Thread.sleep(10);
								...
								thiz[0].keyPressed(new KeyEvent(thiz[0].gameFrame, 0, 0, 0, KeyEvent.VK_ENTER, (char) 10));
								try {
									Thread.sleep(10);
								} catch (InterruptedException e) {
									e.printStackTrace();
								}
								thiz[0].keyReleased(new KeyEvent(thiz[0].gameFrame, 0, 0, 0,  KeyEvent.VK_ENTER, (char) 10));
								try {
									Thread.sleep(10);
								} catch (InterruptedException e) {
									e.printStackTrace();
						...
						thiz[0].isPasting = false;
					}
				}).start();
			}
		}

		if (i == KeyEvent.VK_V && Client.controlIsDown ) {
